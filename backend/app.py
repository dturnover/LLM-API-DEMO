# app.py — FastAPI + cookie sessions + memory + tiny RAG + OpenAI
...
def preflight(path: str): return Response(status_code=204)
...
# ---------- Tiny RAG ----------
@dataclass
class Doc: id: str; page: int; text: str; corpus: str
...
def _ingest(r: Dict[str, Any]):
    ...
    d=Doc(id=str(r.get("id") or f"{c}_{r.get('page',0)}"),
          page=int(r.get("page") or 0), text=t, corpus=c)
    CORPUS[c].append(d)
    ...
    for sub in root.iterdir():
        ...
        if dj.exists():
            try:
                ...
                for r in data: r.setdefault("corpus", sub.name); _ingest(r)
            except: pass
...
async def chat_sse_get(request: Request, q: str):
    ...
    if mem:
        ...
        if auto_corpus:
            ...
            # generic → OpenAI (one-shot then tokenized)
            try:
                if client:
                    r = client.chat.completions.create(
                        model=OPENAI_MODEL,
    ...
    async def gen():
        ...
        if 'final_meta' in locals() and final_meta:
